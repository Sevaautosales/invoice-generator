...
export default function InvoicePreview({ data }: InvoicePreviewProps) {
    ...
    React.useEffect(() => {
        const handleResize = () => {
            const width = window.innerWidth; // 32px padding
            const targetWidth = 850; // The fixed width of the invoice

            if (width < targetWidth + 32) {
                // Calculate scale to fit width minus padding
                const newScale = (width - 32) / targetWidth;
                setScale(newScale);
            } else {
                setScale(1);
            }
    ...
    return (
        <div className="w-full flex justify-center mb-20 print:mb-0 print:block">
            <div
                style={{
                    transform: `scale(${scale})`,
                    transformOrigin: 'top center',
                    height: scale < 1 ? `${1202 * scale}px` : 'auto', // Adjust height to avoid huge whitespace
                    marginBottom: scale < 1 ? `-${1202 * (1 - scale)}px` : '0px' // Negative margin to pull footer up
                }}
                className="transition-transform duration-200 print:transform-none print:h-auto print:mb-0"
            >
                <Card id="invoice-capture-area" className="w-[850px] min-w-[850px] bg-white border-0 print:shadow-none print:border-0 print:p-0 print:m-0 overflow-hidden font-sans text-gray-900 relative print:bg-white" style={{ color: colors.gray900, boxShadow: '0 25px 50px -12px rgba(229, 231, 235, 0.5)' }}>
                    {/* CardContent with A4 aspect ratio matching: 850px width -> ~1202px height */}
                    <CardContent className="p-0 border flex flex-col print:border-0 print:min-h-0 min-h-[1202px]" style={{ borderColor: colors.gray100 }}>
                        ...
                        <div className="flex justify-between items-start px-8 mb-8">
                            ...
                            <div className="text-right">
                                ...
                                <div className="text-sm font-black" style={{ color: '#1a1a1a' }}>99136 55204</div>
